...
# --- Configuration ---
# IMPORTANT: Load keys from environment variables for security (required for Render/Heroku).
BINANCE_API_KEY = os.getenv("BINANCE_API_KEY") # READ FROM ENVIRONMENT
BINANCE_API_SECRET = os.getenv("BINANCE_API_SECRET") # READ FROM ENVIRONMENT
...
client = None
# Check if both keys are available (os.getenv returns None if variable is not set)
if BINANCE_API_KEY and BINANCE_API_SECRET:
    ...
    # Use a public client for fetching data if keys are not set or keys are missing.
    client = Client() 
